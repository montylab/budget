	...
	export default {
			...
			chartData: function () {
				...
				data.forEach(item => {
					...
					if (category.length) {
						...
						if (category[1]) {
							if (!result[category[0]].subs[category[1]]) {
								result[category[0]].subs[category[1]] = {
									type: category[1],
									percent: 0,
									amount: 0,
									value: 0
							...
							result[category[0]].subs[category[1]].amount += item.amount
						}
						result[category[0]].amount += item.amount
					}
				})
		...
		methods: {
			init: function () {
				...
				const types = this.chartData

				function generateChartData() {
					var chartData = [];
					for (var i = 0; i < types.length; i++) {
						if (i == selected && types[i].subs.length) {
							...
							chartData.push({
								...
								id: i,
                                pulled: i == selected && types[i].subs.length==0
							});
						}
					}
				...
				AmCharts.makeChart(this.$el.querySelector('.categoryPieChart'), {
					"type": "pie",
					"theme": "light",

					"dataProvider": generateChartData(),
					"labelText": "[[title]]: [[value]]" + (this.type == 'percent' ? '%' : '$'),
					"balloonText": "[[title]]: [[value]]" + (this.type == 'percent' ? '%' : '$'),
					"titleField": "type",
					"valueField": "value",
					"outlineColor": "#FFFFFF",
					"outlineAlpha": 0.8,
