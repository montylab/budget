<script>
	...
	export default {
        ...
        computed: {
			monthData: function () {
				const data = []
				const income = this.$data.income.data
				const outcome = this.$data.outcome.data
				const timestamps = {
					start: +new Date(),
                    end: +new Date()
                }
				...
				if (income.length) {
					timestamps.start = Math.min(income[0].date)
					timestamps.end = Math.max(income[income.length-1].date)
				}

				if (outcome.length) {
					timestamps.start = Math.min(outcome[0].date)
					timestamps.end = Math.max(outcome[outcome.length-1].date)
				}

                const mon = []
				outcome.forEach(item => {
					const date = new Date(item.date)
					mon[date.getFullYear()] = mon[date.getFullYear()] || []
					mon[date.getFullYear()][date.getMonth()] = mon[date.getFullYear()][date.getMonth()] || {income: 0, outcome: 0}
				    mon[date.getFullYear()][date.getMonth()].outcome += item.amount
				})
				...
				income.forEach(item => {
					...
					mon[date.getFullYear()] = mon[date.getFullYear()] || []
					mon[date.getFullYear()][date.getMonth()] = mon[date.getFullYear()][date.getMonth()] || {income: 0, outcome: 0}
					mon[date.getFullYear()][date.getMonth()].income += item.amount

				})

				for (let year in mon) {
					for (let month in mon[year]) {
						data.push({
							income: mon[year][month].income,
							outcome: -mon[year][month].outcome,
							month: month + " / " + year
						})
					}
				}
		...
		created: function () {
			outcomeService.events.$on('updated', (data) => {
				this.$data.outcome.data = data.items
				this.$data.outcome.categories = data.categories
                this.init()
			})

			incomeService.events.$on('updated', (data) => {
				this.$data.income.data = data.items
				this.$data.income.categories = data.categories
				this.init()
			})
		},

		mounted () {
			this.$data.outcome.data = outcomeService.getItemsArray()
			this.$data.outcome.categories = outcomeService.getCategories()

			this.$data.income.data = incomeService.getItemsArray()
			this.$data.income.categories = incomeService.getCategories()
		...
		methods: {
            init: function () {
				...
				var chart = window.AmCharts.makeChart("inOutChart", {
					"type": "serial",
					"svgIcons": false,
					"categoryField": "month",
					"columnWidth": 0.7,
					...
					"titles": [
						{
							"id": "Title-1",
							"size": 18,
							"text": "Income / Outcome per month",
							"color": '#FFF',
							"fontFamily": "Segoe UI, arial",
						}
					],
					"dataProvider": this.monthData
				});
			}
		}
	}
